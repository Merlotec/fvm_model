...
class FluidVisionModel(nn.Module):
    def __init__(self, num_obs: int, num_patches: int, patch_size: int, emb_dim: int,
        ...
        self.transformer = FluidAxialTransformer(emb_dim, grid_size=grid_size, num_obs=num_obs, num_layers=num_layers)
    ...
    def forward(self, x):
        ...
        x = x[:, -self.num_patches:, :]       # last frame's tokens (B, num_patches, emb_dim)
